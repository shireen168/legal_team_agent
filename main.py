    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        try:
            ...
            embedder = OpenAIEmbedder(
                model="text-embedding-3-small", api_key=st.session_state.openai_api_key
            )
...
def main():
                ...
                with st.spinner("Processing document..."):
                    try:
                        knowledge_base = process_document(
                            uploaded_file, st.session_state.vector_db
                        )
    ...
    if not all([st.session_state.openai_api_key, st.session_state.vector_db]):
        ...
        st.write(
            f"🤖 Active Legal AI Agents: {', '.join(analysis_configs[analysis_type]['agents'])}"
        )  # dictionary!!
